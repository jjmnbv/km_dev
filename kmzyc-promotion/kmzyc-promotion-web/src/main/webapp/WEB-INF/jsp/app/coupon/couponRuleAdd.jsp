...
<script type="text/javascript">
	...
	jQuery.validator.addMethod("checkCouponNameRepeat", function(value, element) {
		 	var rows = 0;
		 		...
		 		var couponName=encodeURI(encodeURI(couponNameOld.replace(/(^\s+)|(\s+$)/g,"")));//去掉空格
		 		var couponId = $("#couponsId").val();
		 		var viewType=$("#viewTypes").val();
				$.ajax({
					async:false,
					url:"/coupon/couponRule_checkCouponName.action?newCouponName="+couponName+"&&couponId="+couponId+"&&viewType="+viewType,
					type:"POST",
					dataType:"json",
	...
	function checkSupplier(){
		 dialog("选择账号","iframe:/coupon/couponRule_checkSupplier.action" ,"900px","450px","iframe");
...
<body>
	...
	<s:form action="/coupon/couponRule_saveCoupon.action" method="POST" namespace='/app' id="couponForm">
	<s:token></s:token>
		<!--   隐藏数据域!-->
			...
			<tr >
			 <td colspan="4" align="right" class="eidt_rowTitle"  >
				<table width="97%" align="center" >
					...
					<tr>
						<td><span class="eidt_rowTitle">使用限制：</span></td>
						<td>
							<s:if test="viewType=='add' ||viewType=='view'">
								<input type="checkbox"  name="useLimitsTypes" value="1" <s:if test="coupon.useLimitsType==1">checked="checked"</s:if> /><span>不可和余额同时使用</span>
							</s:if>
							 <s:if test="viewType=='show'">
								<input type="checkbox" disabled="disabled" <s:if test="coupon.useLimitsType==1">checked="checked"</s:if> /><span>不可和余额同时使用</span>
							</s:if>
						</td>
					</tr>
					<tr>    
