...
public class KeywordFliter {
	// 日志记录
	protected static Logger logger = LoggerFactory.getLogger(KeywordFliter.class);
	...
	public static void main(String[] args) {
		...
		Date begin = new Date();
		/*
		 * for(int i=0;i<keyWords.length;i++){ if(txt.indexOf(keyWords[i])!=0){
		 * 
		...
		// 判断文本是否含有关键字
		// System.out.print(filter.isContentKeyWords(txt));
	...
	public static void printCostTime(String name, Date begin) {
		Date now = new Date();
		logger.info(name + (now.getTime() - begin.getTime()));
	}
