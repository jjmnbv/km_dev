...
import com.kmzyc.promotion.sys.dao.SysUserDAO;
import com.kmzyc.promotion.sys.dao.SysUserDAOImpl;
import com.kmzyc.promotion.sys.dao.SysUserRoleDAOImpl;
import com.kmzyc.promotion.sys.model.SysRole;
...
import com.kmzyc.promotion.sys.model.SysUserRoleExample;
...
public class SysUserBean extends BaseBean {
    ...
    public Page searchProductUserListPage(Page pageParam, SysUser vo) throws Exception {
        int pageNo = pageParam.getPageNo();// 当前查询第几页
        if (pageNo == 0)
            pageNo = 1;// 首次查询第一页
        int pageSize = pageParam.getPageSize(); // 每页显示记录数
        ...
        try {
            ...
            vo.setSkip(skip);
            vo.setMax(max);
            page = dao.selectProductUsersPageByVo(pageParam, vo);
            page.setPageNo(pageNo);// 当前查询第几页
            vo.setUserReal(userReal);
