...
public class SearchConfig {
    ...
    public static SearchConfig build(Map<String, String[]> httpParameters) {
        ...
        // 物理类目
        String[] code = searchconfig.getConfig().remove(HTTPParam.code.name());
        if (ArrayUtils.isNotEmpty(code) && StringUtils.isNotBlank(code[0])) {
            if (StringUtils.isNotBlank(queryText.toString())) {
                queryText.append(" AND ");
            }
            queryText.append(DocFieldName.PHYSICS_CODE + ":" + code[0]);
        }
        // 品牌id
