...
public class BaseController extends AbstractController {
    ...
    protected void addModel(ModelAndView view, Map<String, Object> result) {
        ...
        // 已选过滤参数
        Map<String, Map<String, String>> filters = getFilter();
        ...
        if (null != facterList && !facterList.isEmpty()) {
            ...
            for (Facter f : facterList) {
                if (filters.containsKey(f.getCode())) {
                    continue;
                }
                newfacterList.add(f);
            }
    ...
    /**
     * brandName=品牌值&price=价格值&属性名=属性值
     * 
     * @return
     */
    protected Map<String, Map<String, String>> getFilter() {
        ...
        if (StringUtils.isNotBlank(filterText)) {
            ...
            while (it.hasNext()) {
                ...
                if (HTTPParam.brandName.name().equals(pName)) {
                    paramValue.put("品牌", pValue);
                } else if (HTTPParam.price.name().equals(pName)) {
                    paramValue.put("价格", pValue);
                } else if (HTTPParam.attr.name().equals(pName)) {
                    String[] temp = pValue.split("_");
                    paramValue.put(temp[0], temp[1]);
                }
                filterParam.put(pName, paramValue);
            }
        }
        return filterParam;
    }
