...
public class AjaxB2BSearchAction extends BaseController {
    ...
    private String getCategory(final String skuId) {
        ...
        try {
            ...
            if (null == resultMap || resultMap.isEmpty()) {
                ...
                Map<Integer, Map<String, Object>> secondCategroyMap =
                        (Map<Integer, Map<String, Object>>) resultMap.get(1);
                returnResult.setReturnObject(infoMap);
                returnResult.setSecondCategroyMap(secondCategroyMap);
    ...
    private Map<Integer, Object> getResult(String skuId) {
        ...
        try {
            ...
            if (null != infoMap && !infoMap.isEmpty()) {
                ...
                // 有第三级运营类目，必然有1，2级运营类目
                getSecondCategorys(infoMap, categories, resultMap);
                return resultMap;
            }
    ...
    // 封装一级运营类目所属的所有二级运营类目，用于明细页“热门分类”
    private void getSecondCategorys(Map<Integer, Map<String, Object>> infoMap,
            Map<Integer, Category> categories, Map<Integer, Object> resultMap) {
        Object categoryFirstId = infoMap.get(0).get("oid");// 一级运营类目
        Map<Integer, Map<String, Object>> secondCategroyMap = Maps.newHashMap();
        int count = 0;
        for (Category cate : categories.values()) {
            if (Integer.parseInt(categoryFirstId.toString()) == cate.getParentId()) {
                Map<String, Object> map = new HashMap<String, Object>();
                map.put("name", cate.getName());
                map.put("oid", cate.getId());
                secondCategroyMap.put(count, map);
                count++;
            }
        }
        resultMap.put(1, secondCategroyMap);
    }
