...
import com.kmzyc.search.facade.vo.ModelAttribute;
...
public class B2BWebSearchAction extends BaseController {
    ...
    public ModelAndView categorySearch() {
        ...
        if (!containts(HTTPParam.code.name())) {
            LOG.warn("搜索物理类目code为空！");
            setViewName(view, JSPResource.unmatchSearch);
            return view;
        }
        Map<String, Object> result = Maps.newHashMap();
        try {
            ...
            String code = getParameter(HTTPParam.code.name());
            if (StringUtils.isNotBlank(code)) {
                // 生成面包屑路径
                String prefix = "categorySearch?code=" + code;
                getBreadMapAndHot1(result, prefix);

                // 获取类目名称用于页面title描述
                String categoryName =
                        searchDBDataService.getCategoryNameById(Integer.parseInt(code));
            ...
            LOG.error("获取物理类目:{} 搜索结果失败!", getParameter(HTTPParam.code.name()), e);
            view.setViewName(getErrorViewName());
        }
        addModel(view, result);
        return view;
    ...
    public ModelAndView shopSearch() {
        ...
        try {
            ...
            if (StringUtils.isNotBlank(shopCode)) {
                ...
                // 获取供应商名称用于页面title描述
                String supplier = searchDBDataService.getSupplierById(Integer.parseInt(shopCode));
                if (StringUtils.isNotBlank(supplier)) {
                    setAttribute("title", supplier + "-商品搜索-康美中药城");
                    setAttribute("keywords", supplier + "价格，" + supplier + "说明书，" + supplier + "效果，"
                            + supplier + "作用");
                }
            }
        } catch (Exception e) {
    ...
    protected RecursiveTask<List<ProductItem>> startHotTask() {
        RecursiveTask<List<ProductItem>> hotTask = new RecursiveTask<List<ProductItem>>() {

            /**
             * 
             */
            private static final long serialVersionUID = 4256099247635618579L;

            @Override
            protected List<ProductItem> compute() {
                List<ProductItem> products = otherSystemService.getHotSearchProducts();
                return products;
            }
        };
        hotTask.fork();
    ...
    private void getBreadMapAndHot1(Map<String, Object> result, String prefix) {
        ...
        try {
            ...
            RecursiveTask<List<ProductItem>> hotTask = startHotTask();

            Map<String, String> breadMap = getBreadVal(prefix, Channel.b2b);
            result.put(ModelAttribute.breadMap.name(), breadMap);

            result.put(ModelAttribute.hotSearchPaoduct.name(), hotTask.join());
        } catch (Exception e) {
