...
<div class="i-filter fn-clear">
	<ul id="sortBar" class="filter-order">
		<c:choose>
        	<c:when test="${sort == 5 }">
				...
				<%-- <li><a href="${km:qsReplace(km:qsRemove(searchURL, 'pn'), 'sort', '10') }">积分</a></li> --%>
			</c:when>
	       	<c:when test="${sort == 6 }">
	        	<li><a href="${km:qsRemove(km:qsRemove(searchURL, 'pn'), 'sort') }">综合</a></li>
			...
			<c:when test="${sort == 3 }">
             ...
             <c:when test="${sort == 4 }">
				  ...
				  <c:when test="${sort == 9 }">
					...
					<li><a href="${km:qsReplace(km:qsRemove(searchURL, 'pn'), 'sort', '1') }">价格</a></li>
					<li><a href="${km:qsReplace(km:qsRemove(searchURL, 'pn'), 'sort', '4') }">时间</a></li>
					<%-- <li class="current contrl-upward"><a href="${km:qsReplace(km:qsRemove(searchURL, 'pn'), 'sort', '10') }">积分</a></li> --%>
			</c:when>
			<c:when test="${sort == 10 }">
                    <li><a href="${km:qsRemove(km:qsRemove(searchURL, 'pn'), 'sort') }">综合</a></li>
					<li><a href="${km:qsReplace(km:qsRemove(searchURL, 'pn'), 'sort', '6') }">销量</a></li>
					<li><a href="${km:qsReplace(km:qsRemove(searchURL, 'pn'), 'sort', '1') }">价格</a></li>
					<li><a href="${km:qsReplace(km:qsRemove(searchURL, 'pn'), 'sort', '4') }">时间</a></li>
					<%-- <li class="current"><a href="${km:qsReplace(km:qsRemove(searchURL, 'pn'), 'sort', '9') }">积分</a></li> --%>
			</c:when>
			<c:otherwise>
	...
	<c:choose>
   	...
   	&nbsp;&nbsp;&nbsp;
   	<!-- maliqun add  20150625 增加康美自营产品筛选项 begin -->
   	<!-- isKmSelf 在RequestParamsTransverter.java的setFilter方法中需要将isKmSelf转换成index中能够识别的suptype以及scode, 1,3&scode=211  suptype=1,3为供应商的自营代销类型,221为km自营代号-->
   	<c:choose>
   	...
   	<!-- maliqun add  20150625 增加康美自营产品筛选项 end -->
   	
   	
   
	
	<div class="filter-total fn-right" id="topPagination">
	<span>共<strong>${count }</strong>个商品</span>
	<span class="text"><i>${pn}</i>/${pc}</span>
	<c:choose>
   		<c:when test="${pn eq 1 }">
