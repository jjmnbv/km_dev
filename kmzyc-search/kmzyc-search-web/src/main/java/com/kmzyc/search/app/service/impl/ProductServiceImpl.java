...
public class ProductServiceImpl implements ProductService {
    ...
    private void extractSKUInfo(Map<String, Object> recodes, long skuId) {
        ...
        sql.append(
                "select ps.PRODUCT_SKU_ID, ps.PRODUCT_SKU_CODE, ps.PRODUCT_ID, ps.PRICE, ps.MARK_PRICE, p.CHANNEL, ps.PV_VALUE from PRODUCT_SKU ps, PRODUCTMAIN p");
        sql.append(
                " where ps.PRODUCT_ID=p.PRODUCT_ID and p.STATUS=3 and ps.status=1 and ps.PRODUCT_SKU_ID="
                        + skuId);
        try {
            ...
            if (!skuInfo.isEmpty()) {
                ...
                Double price = MapUtils.getDoubleValue(skuInfo, "PRICE"); // 商品销售价格
                recodes.put(DocFieldName.SPRICE, price);
                recodes.put(DocFieldName.PRICE, price);
                /*
                 * String channel = MapUtils.getString(skuInfo, "CHANNEL"); Double pm_price =
                 * promotionService.promotionPrice(id, price, channel); // 判断商品是否参加促销活动，并获取促销价格 if
                 * (null != pm_price) { // 参加促销活动 recodes.put(DocFieldName.PRICE, pm_price); //
                 * 销售价格替换为促销价格 recodes.put(DocFieldName.PROMOTION, 1); }
                 */
                ...
                recodes.put(DocFieldName.SKUPV, MapUtils.getDoubleValue(skuInfo, "PV_VALUE"));
            }
    ...
    private void extractSKUNavAttr(Map<String, Object> recodes, long skuId) {
        ...
        sql.append(
                " where psa.CATEGORY_ATTR_VALUE_ID=cav.CATEGORY_ATTR_VALUE_ID(+) and ca.CATEGORY_ATTR_ID = psa.CATEGORY_ATTR_ID and PRODUCT_SKU_ID="
                        + skuId);
        ...
        try {
            ...
            if (!skuAttrInfo.isEmpty()) {
                ...
                attrName = null == attrName ? new ArrayList<String>(skuAttrInfo.size()) : attrName;

                List<String> attrValue = (List<String>) recodes.get(DocFieldName.ATTRVAL);
                attrValue =
                        null == attrValue ? new ArrayList<String>(skuAttrInfo.size()) : attrValue;

                List<String> attrValueCP = (List<String>) recodes.get(DocFieldName.ATTRVAL_CP);
                attrValueCP = null == attrValueCP ? new ArrayList<String>(skuAttrInfo.size())
                        : attrValueCP;
    ...
    private void extractAllSKUAttr(Map<String, Object> recodes, final long skuId) {
        ...
        String sql =
                "select psa.CATEGORY_ATTR_ID, psa.CATEGORY_ATTR_NAME, decode(cav.category_attr_value_id,null,psa.category_attr_value_id,cav.category_attr_value_id) category_attr_value_id, decode(psa.new_attr,null,cav.category_attr_value,psa.new_attr) category_attr_value from PRODUCT_SKU_ATTR psa, CATEGORY_ATTR ca, CATEGORY_ATTR_VALUE cav where psa.CATEGORY_ATTR_VALUE_ID=cav.CATEGORY_ATTR_VALUE_ID(+) and ca.CATEGORY_ATTR_ID = psa.CATEGORY_ATTR_ID and PRODUCT_SKU_ID=?";
    ...
    private Map<String, Object> extractSupplierInfo(final Map<String, Object> recodes) {
        ...
        String supSql =
                "select su.SUPPLIER_TYPE, su.SUPPLIER_ID from suppliers_info su where su.SUPPLIER_ID=?";
        ...
        jdbcTemplate.query(supSql, new PreparedStatementSetter() {
            ...
            @Override
            public Object mapRow(ResultSet rs, int rowNum) throws SQLException {
                int type = rs.getInt("SUPPLIER_TYPE");
                recodes.put(DocFieldName.SUPTYPE, type);
                return null;
            }
        });
        return recodes;
    ...
    public void batchAddB2bWhole(List<Map<String, Object>> productList) {
        // 插入sql
        String sql =
                "insert into SEARCH_B2B_DATA_WHOLE(PRODUCT_SKU_ID,PRODUCT_ID,PRODUCT_SKU_CODE,PRICE,MARK_PRICE,PV_VALUE,PROCUCT_NAME,"
                        ...
                        + "SUPPLIER_TYPE,SHOP_ID,SHOP_NAME,PRODUCT_ATTR_NAME_OP,PRODUCT_ATTR_NAME_OPVAL,QUALITY,GRADE,PNUM,BROWSE,SHOP_CATEGORY_ID) "
                        + "values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
        ...
        jdbcTemplate.batchUpdate(sql, new BatchPreparedStatementSetter() {
            ...
            public void setValues(PreparedStatement ps, int i) throws SQLException {
                ...
                ps.setObject(6, tempList.get(i).get("skupv"));
                ps.setObject(7, tempList.get(i).get("prodName"));
                ps.setObject(8, tempList.get(i).get("prodTitle"));
                ps.setObject(9, tempList.get(i).get("subtitle"));
                ps.setObject(10, tempList.get(i).get("keyword"));
                ps.setObject(11, tempList.get(i).get("prodHot"));
                ps.setObject(12, tempList.get(i).get("brandId"));
                ps.setObject(13, tempList.get(i).get("brandName"));
                ps.setObject(14, tempList.get(i).get("cTime"));
                ps.setObject(15, tempList.get(i).get("upTime"));
                ps.setObject(16, tempList.get(i).get("approType"));
                ps.setObject(17, tempList.get(i).get("approNum"));
                ps.setObject(18, tempList.get(i).get("scode"));
                ps.setObject(19, tempList.get(i).get("pno"));
                ps.setObject(20, tempList.get(i).get("drugId"));
                ps.setObject(21, tempList.get(i).get("drugCode"));
                ps.setObject(22, tempList.get(i).get("channel"));
                ps.setObject(23, tempList.get(i).get("prodType"));
                ps.setObject(24, tempList.get(i).get("sales"));

                String[] images = (String[]) tempList.get(i).get("image");
                ps.setObject(25, concatStr(images, ","));

                List<Object> attrnames = (List<Object>) tempList.get(i).get("attrname");
                ps.setObject(26, concatObjStr(attrnames, ","));

                List<Object> attrvals = (List<Object>) tempList.get(i).get("attrval");
                ps.setObject(27, concatObjStr(attrvals, ","));

                List<Object> attrval_cps = (List<Object>) tempList.get(i).get("attrval_cp");
                ps.setObject(28, concatObjStr(attrval_cps, ","));

                List<Object> san_ss = (List<Object>) tempList.get(i).get("san_ss");
                ps.setObject(29, concatObjStr(san_ss, ","));

                List<Object> sav_cns = (List<Object>) tempList.get(i).get("sav_cns");
                ps.setObject(30, concatObjStr(sav_cns, ","));

                List<Object> sav_ss = (List<Object>) tempList.get(i).get("sav_ss");
                ps.setObject(31, concatObjStr(sav_ss, ","));

                String[] pcIds = (String[]) tempList.get(i).get("pcId");
                ps.setObject(32, concatStr(pcIds, ","));

                String[] pcCodes = (String[]) tempList.get(i).get("pcCode");
                ps.setObject(33, concatStr(pcCodes, ","));

                String[] pcNames = (String[]) tempList.get(i).get("pcName");
                ps.setObject(34, concatStr(pcNames, ","));

                ps.setObject(35, tempList.get(i).get("supType"));
                ps.setObject(36, tempList.get(i).get("shopId"));
                ps.setObject(37, tempList.get(i).get("shopName"));

                List<Object> opnames = (List<Object>) tempList.get(i).get("opname");
                ps.setObject(38, concatObjStr(opnames, ","));

                List<Object> opvals = (List<Object>) tempList.get(i).get("opval");
                ps.setObject(39, concatObjStr(opvals, ","));

                ps.setObject(40, tempList.get(i).get("stock"));
                ps.setObject(41, tempList.get(i).get("grade"));
                ps.setObject(42, tempList.get(i).get("pnum"));
                ps.setObject(43, tempList.get(i).get("browse"));

                Integer[] shopgorys = (Integer[]) tempList.get(i).get("shopgory");
                ps.setObject(44, concatStr(shopgorys, ","));
            }
    ...
    public void batchUpdateB2bWhole(List<Map<String, Object>> productList) {
        // 插入sql
        String sql =
                "update SEARCH_B2B_DATA_WHOLE set PRODUCT_ID=?,PRODUCT_SKU_CODE=?,PRICE=?,MARK_PRICE=?,PV_VALUE=?,PROCUCT_NAME=?,PRODUCT_TITLE=?,"
        ...
        jdbcTemplate.batchUpdate(sql, new BatchPreparedStatementSetter() {
            ...
            public void setValues(PreparedStatement ps, int i) throws SQLException {
                ...
                ps.setObject(5, tempList.get(i).get("skupv"));
                ...
                ps.setObject(44, tempList.get(i).get("id"));
            }
