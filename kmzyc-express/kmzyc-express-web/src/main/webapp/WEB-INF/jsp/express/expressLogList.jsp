...
<html>
	...
	<body>
		...
		<form id="logListSearch" action="expressLog_pageList.action" method="POST">
		<table class="table_search" width="90%" align="center" cellpadding="0" cellspacing="0" >
			<tr>
				<td>
					订单号：<input class="condition" name="map['orderCode']" type="text" value='<s:property value="map['orderCode']"/>'>
				</td>
		        <td>
		           	物流单号：<input class="condition" name="map['logisticsNo']" type="text" value='<s:property value="map['logisticsNo']"/>'>
				</td>
				<td align="right"></td>
		        <td>
				...
				<td align="right">
					...
					<select class="condition" id="node" name="map['node']" style="width:156px">
				   		...
				   		<option value="1" <s:if test='map["node"]==1'>selected="selected"</s:if> >接收物流信息</option>
				   		<option value="2" <s:if test='map["node"]==2'>selected="selected"</s:if> >提交物流单号至快递100</option>
				   		<option value="3" <s:if test='map["node"]==3'>selected="selected"</s:if> > 同步已发货订单</option>
				   </select>
				</td>
			</tr>
			<tr>		
		...
		<table class="list_table" width="98%" align="center" cellpadding="3" cellspacing="0" bgcolor="#f2f8ff" border="1" bordercolor="#0099cc" style="border-collapse: collapse;font-size:12px">
			...
			<tr>
		   		 ...
		   		 <td><s:property value="#log.statusName"/></td>
			</tr>
			</s:iterator>
		</table>
		<br/>
