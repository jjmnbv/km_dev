...
<html>
	...
	<body>
		...
		<form id="subscriptionListSearch" action="expressSub_pageList.action" method="POST">
		<table class="table_search" width="90%" align="center" cellpadding="0" cellspacing="0" >
			...
			<tr>
				<td align="left">
				   渠道&nbsp;&nbsp;&nbsp;：<select class="condition" id="channel" name="map['channel']" style="width:156px">
					   		<option value=""    <s:if test="map['channel']==''">selected="selected"</s:if> >全部</option>
					   		<option value="B2B" <s:if test="map['channel']==B2B">selected="selected"</s:if> >B2B</option>
					   		<option value="B2C" <s:if test="map['channel']==B2C">selected="selected"</s:if> >B2C</option>
				   		</select>
				</td>
				<td align="left">
				   推送状态：<select class="condition" id="latestPushStatus" name="map['latestPushStatus']" style="width:156px">
		        ...
		        <td align="left">
		           	快递状态：<select class="condition" id="expressStatus" name="map['expressStatus']" style="width:156px">
					   		...
					   		<option value="0" <s:if test="map['expressStatus']==0 && map['expressStatus']!=''">selected="selected"</s:if> >在途中</option>
					   		<option value="1" <s:if test="map['expressStatus']==1">selected="selected"</s:if> >已揽收</option>
					   		<option value="2" <s:if test="map['expressStatus']==2">selected="selected"</s:if> >疑难</option>
					   		<option value="3" <s:if test="map['expressStatus']==3">selected="selected"</s:if> >已签收</option>
				   		</select>
				</td>
				<td align="left">
				...
				<td align="left">&nbsp;&nbsp;
					...
					<input type="button" onclick="insertSub()"   value="新增订阅"/>&nbsp;
