package com.kmzyc.framework.fileop;
...
public class FileUtil {
    ...
    public static void deleteDir(File dir) throws IOException {
        ...
        if (files != null) {
            for (int i = 0; i < files.length; i++) {
                ...
                if (file.isFile()) {
                    if (!file.delete())
                        throw new IOException(String.format("file %s delete failure", file.getAbsolutePath()));
                } else {
                    deleteDir(file);
                }
            }
        ...
        if (!dir.delete())
            throw new IOException(String.format("file %s delete failure", dir.getAbsolutePath()));
    }
    ...
    /**
     * provider file path <code>d:/filepath</code>
     * to add content <code>this is context !</code> .
     * if file not found then create new file to add.
     *
     * @throws IOException
     * @throws if          error throws exception.
     */

    public static void appendContextToFile(String path, String content) throws IOException {
        File file = new File(path);
        if (!file.exists()&&!file.createNewFile()) {
           throw new IOException(String.format("file %s not exists or create failure",path));
        }
        ...
        ByteBuffer bf = ByteBuffer.wrap(content.getBytes("UTF-8"));
        filechenel.write(bf);
        filechenel.close();
        fis.close();
