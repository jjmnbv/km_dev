...
function checkCustNameAjax() {
	...
	var result = $.ajax({
		url : "/cust/checkCustName.action?custName="
				+ encodeURI(custName.value),
		async : false
	}).responseText;
...
function checkPwd() {
	...
	if (!check_text(password, 6, 15, "长度不符，应在6-15个字符间！", divpwd)
			|| !check1(password, divpwd)) {
		return false;
	} else {
...
function checkPassword() {
	...
	if (!check_text(pwd, 6, 15, "长度不符，应在6-15个字符间！", d_pwd)
			|| !check1(pwd, d_pwd)) {
		return false;
	} else {
		var result = $.ajax({
...
function check_length_asc(Item, MinLength, MaxLength, ItemCaption, ItemShow) {
	...
	if ((Item.value.Tlength() < MinLength)
			|| (Item.value.Tlength() > MaxLength)) {
		ItemShow.className = "d_err";
