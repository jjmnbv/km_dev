...
<script type="text/javascript">
	function turnDetail(obj){
		...
		window.location.href="/app/toDetailOrder.action?sno=" + sno + "&toType=cai";
	};
...
<body>
	...
	<s:set name="name" value="' 结算单-财务审核'" scope="request" />
	<s:set name="son_name" value="'列表'" scope="request" />
	<s:include value="/WEB-INF/jsp/public/title.jsp" />
	<form action="finacialAuditList.action?flag=3" method="POST">
		...
		<table width="98%" class="table_search" align="center" cellpadding="0"
			...
			<tr>
				...
				<td><input value="<s:property value="sellerSettlementCriteria.settlementNo" />" id="settlementNo" name="_settlementNo" type="text"></td>
				...
				<td><input name="_startDate" id="startDate"
					...
					class="Wdate"
					onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'#F{$dp.$D(\'endDate\')}'})" /><strong>&nbsp;至&nbsp;</strong>
					<input name="_endDate" id="endDate"
				...
				<td>
				<select name="_settlementStatus" id="settlementStatus">
				...
				<td align="center">
					<INPUT TYPE="submit" class="btn-custom button-blue-1"value=" 查询 "/>
		...
		<table class="list_table" width="98%" align="center" cellpadding="3"
			...
			<c:if test="${not empty dataList}">
				...
				onMouseOut="this.style.backgroundColor='#FFFFFF'">
					...
					<td><fmt:formatDate value="${item.settlementCreateTime }" pattern="yyyy-MM-dd HH:mm:ss" /></td>
					<td><fmt:formatNumber type="NUMBER" value="${item.receiveSum-item.refundSum }" pattern="#0.00#"></fmt:formatNumber></td>
					<td><fmt:formatNumber type="NUMBER" value="${item.fareSum }" pattern="#0.00#"></fmt:formatNumber></td>
					<td>
					...
					<td>
				  		   <c:if test="${item.settlementStatus ==1  }"> 未确认</c:if>
						   ...
						   <c:if test="${item.settlementStatus ==6  }"> 已结出</c:if>
