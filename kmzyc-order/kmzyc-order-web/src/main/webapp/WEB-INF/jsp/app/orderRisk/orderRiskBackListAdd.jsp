	    ...
	    $('#btnsub').bind('click', function(){
		   ...
		   /*  if(type==2 && !/^(13[0-9]|15[0|3|6|7|8|9]|18[8|9])\d{8}$/.test(name)){
			...
			if(name==""||$.trim(name).length==0){
				alert('黑名单内容不能为空');
			}else if(confirm('确认将'+name+'添加到风控黑名单？')){
				$.ajax({
		            async: false,
		            url: '/app/addBlackList.action',
		            cache:false,
		            type:'post',
...
<table width="93%" class="edit_table" cellpadding="3" cellspacing="0" border="1" bordercolor="#C7D3E2" style="border-collapse: collapse;margin-left:10px;margin-top:10px;">
		<tbody><tr>
			<th align="left" class="edit_title">添加风控黑名单条目</th>
		</tr>
        <tr>
        <td>请选择黑单条目类型：</td>
        </tr>
		...
		<tr>
		<td>
			<input type="text" id="riskContent" name="riskContent" value="下单人用户名、手机号码或者商家用户名" maxlength="30"style="color:#ccc; width: 266px;" onclick="if(value==defaultValue){value='';this.style.color='#333'}" onblur="if(!value){value=defaultValue;this.style.color='#ccc'}" />
			</td>
		</tr>
		<tr>
			<td align="center">
