     ...
     <div class="row-fluid">
         <jsp:include page="/WEB-INF/jsp/common/menubars/leftMenu_activity.jsp"></jsp:include>
                ...
                <div class="content">
					...
					<input type="hidden" id="supplierEntryId" value="<s:property value="supplierEntryId"/> " >
					<input type="hidden" id="skuId" value="<s:property value="skuId"/>" >
					...
					<div class="orders">
                        ...
                        <button class="btn btn-danger backBtn" id="return" 
					...
					<tr>
						 <td>						 
						  <table width="100%" border="0" class="newform">
		                    <tbody>
		                      <tr>
		                        <td width="120px"><a target="_blank" href="<s:property value='cmsPagePath'/><s:property value='#orderItem.productSkuId'/>.shtml" class="pull-left"><img class="thumbnail" src="<s:property value="imagePath"/><s:property value="#orderItem.imageUrl" />"></a></td>
		                        <td class="textc"><a target="_blank" href="<s:property value='cmsPagePath'/><s:property value='#orderItem.productSkuId'/>.shtml" title="<s:property value="#orderItem.commodityTitle"/>"><s:property value="#orderItem.commodityTitle"/></a></td>
		                      </tr>
		                    </tbody>
		                  </table>
		                 </td>	   	 	
				   	 	<td class="width100"><s:property value="#orderItem.commoditySkuDescription"/></td>
				   	 	 <td class="width100"><a href="<s:property value='cmsPagePath'/><s:property value='#orderItem.productSkuId'/>.shtml" class="fn-blue" target="_blank"><s:property value="#orderItem.commoditySku"/></a></td>
				   	 	 <td class="width100"><s:property value="#orderItem.sellerSkuCode"/></td>
				   	 	 <td class="width100"><s:property value="#orderItem.commodityUnitPrice"/></td>
				   	 	 ...
				   	 	 <td class="width100"><s:property value="#orderItem.commodityUnitPrice * #orderItem.commodityNumber"/></td>
				   	 	 <td class="width100"><s:property value="#orderItem.commodityUnitIncoming * #orderItem.commodityNumber"/></td><!--
				   	 	 <s:if test="#session.seesionKmb2bSupplierType == 4">
...
<thead>
<tr class="tablesbg">
<th colspan="15"><div class="moneyli">
                        <ul>
                        <li><label>商品总额:</label><p <s:if test="order.commoditySum<=0">class="fontgay"</s:if>><s:property value="order.commoditySum"/></p> 元</li>
                        <li><label>商品运费:</label><p <s:if test="order.fare<=0">class="fontgay"</s:if>><s:property value="order.fare"/></p> 元</li>
                        
                        <s:if test="0!=plusDiscount">
						<li><label>加价购:</label><p <s:if test="plusDiscount<=0">class="fontgay"</s:if>><s:property value="plusDiscount"/></p> 元</li>
					    </s:if>
						<s:if test="0!=fullDdiscount">
						 <li><label>满减:</label><p <s:if test="fullDdiscount<=0">class="fontgay"</s:if>><s:property value="fullDdiscount"/></p> 元</li>
					    </s:if>
					     <li><label>应付金额:</label><p <s:if test="actualpay<=0">class="fontgay"</s:if>><s:property value="actualpay"/></p> 元</li>
					     <li><label>已付金额:</label><p <s:if test="orderpay<=0">class="fontgay"</s:if>> <s:property value="orderpay"/></p> 元</li>
                        <s:if test="0!=couponpay">
                        	 <li><label>优惠券:</label><p <s:if test="couponpay<=0">class="fontgay"</s:if>> <s:property value="couponpay"/></p> 元</li>
					    </s:if>
					    <s:if test="0!=reservepay">
					     <li><label>预备金:</label><p <s:if test="reservepay<=0">class="fontgay"</s:if>><s:property value="reservepay"/></p> 元</li>
					    </s:if>
					    <s:if test="0!=balancepay">
					     <li><label>余额支付:</label><p <s:if test="balancepay<=0">class="fontgay"</s:if>><s:property value="balancepay"/></p> 元</li>
					    </s:if>
					    <s:if test="0!=bankpay">
					     <li><label>网银/信用卡:</label><p <s:if test="bankpay<=0">class="fontgay"</s:if>><s:property value="bankpay"/></p> 元</li>
					    </s:if>
					    <s:if test="0!=platformpay">
					     <li><label>在线:</label><p <s:if test="platformpay<=0">class="fontgay"</s:if>><s:property value="platformpay"/></p> 元</li>
					    </s:if>
					    <s:if test="0!=onlinepay">
					   		<li><label>在线:</label><p <s:if test="onlinepay<=0">class="fontgay"</s:if>><s:property value="onlinepay"/></p> 元</li>
						</s:if>
						<li><label>未付金额:</label><p <s:if test="notpay<=0">class="fontgay"</s:if>><s:property value="notpay<0?0.00:notpay"/></p> 元</li><!--
		...
		<div class="ui-form ui-form-info fn-mt20">
			<fieldset>
				...
				<div class="ui-form-item">
					<label class="ui-form-label" for=""><font color="red">*</font>物流单号：</label>
					<input class="ui-input logisticsNo" type="text" style="width: 140px;"
						value="<s:property value='order.logisticsNo'/>"
						maxlength="30" />
					<p class="ui-form-tiptext ui-tiptext-error ui-form-inline fn-ml10 j_logisticsNo_error" style="display: none;">
						<i class="ui-icon ui-icon-error"></i><span class="logisticsNo_error_msg" style="font-size:12px;"></span>
					</p>
