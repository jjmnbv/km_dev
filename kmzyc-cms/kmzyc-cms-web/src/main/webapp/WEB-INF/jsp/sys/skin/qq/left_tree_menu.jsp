	 ...
	 %>
			<div class="menubox">
				<div class="menuTitle">
					...
					<img src="/etc/images/sys/menu_on.png"   id="image<%=menu.getMenuId()%>" name="image<%=menu.getMenuId()%>" onclick="return false;" />
					</div>
					<div class="menutext">
						...
						<%}else{ %>
							<span class="menutexts" style="cursor:pointer;"  onclick="showmenu('ff<%=menu.getMenuId()%>',image<%=menu.getMenuId()%>);" ><%=menu.getMenuName()%></span>
						<%} %>
					</div><!--
					<div class="menupic">
				...
				%>
				   <ul>
						<li>
							<p>
							<%if(subMenu.getMenuLink().contains("www")){ %>
								<a href="<%=subMenu.getMenuLink()%>" class="menutexts" target="_blank"><%=subMenu.getMenuName()%></a>
							<%}else{ %>
