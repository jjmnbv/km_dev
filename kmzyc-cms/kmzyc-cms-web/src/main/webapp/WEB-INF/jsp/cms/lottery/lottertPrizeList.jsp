...
<html>
	...
	for(i=1;i<=tabLength;i++)
	{
		...
		if(objname.value==null||objname.value=='')
		{
			...
			var objurl = document.getElementById("dataurl"+i);
              	 ...
              	 if(mychecks==0){
                  	...
                  	$.ajax({
                  			 ...
                  			 var myData = eval("("+data+")");
                  			 	 $str='';
         				         $str+="<tr class='dataTr'>";											     
         				         $str+="<td  style='display:none'><input checked='checked'  type='checkbox'  name='dataIds' value='"+myData['awardsId']+"' onclick=checkboxChange(); /></td>";
         				      	 $str+="<td>";
         				         if(typeof myData["title"]!="undefined")
         				      	 {$str+=myData["title"];}
         				         $str+="</td>";
         				         $str+="<td>";
        				         if(typeof myData["probability"]!="undefined")
        				      	 {$str+=myData["probability"];}
        				        $str+="</td>";
         				         $str+="<td>";
                                 if(typeof myData["awardsCount"]!="undefined")
        				      	 {$str+=myData["awardsCount"];}
        				         $str+="<td><input type='text' id='datatext"+i+"' name='cmsWindowDatas["+i+"].user_defined_name'/></td>";
         				     
         				      	 
         				      	 $str+="<td><input type='text' id='dataurl"+ i +"' name='cmsWindowDatas["+i+"].user_defined_url'/></td>";
         				      	
         				      	 
         				         $str+="<td><input type='hidden' name='cmsWindowDatas["+i+"].dataType'  value='10'/>"+
         				         "<input type='hidden' name='cmsWindowDatas["+i+"].dataId' value='"+myData['awardsId']+"'/>"+
         				         "<img title='删除' style='cursor: pointer;' src='/etc/images/icon_delete.png'   onclick='deleteByKey(this);' /></td>";  
         				         $str+="</tr>";
 ...
 function gotoList(){
	...
	<body onload="loadData();">
		...
		<s:form name="windowDataForm" action="/cms/cmsWindowDataAction_add.action"   onsubmit="return check();"
			 method="post">
			<s:token></s:token>
			<!-- 查询条件 -->
