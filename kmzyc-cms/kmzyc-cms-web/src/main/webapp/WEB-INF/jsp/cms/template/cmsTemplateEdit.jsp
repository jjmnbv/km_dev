...
<table width="95%" class="edit_table" cellpadding="3" cellspacing="0" border="1" bordercolor="#C7D3E2" style="border-collapse: collapse">
	...
	<tr> 
		...
		<td width="80%">
	        ...
	        <option value="2" <s:if test="cmsTemplate.type==2">selected="selected"</s:if>>供应商广告模板</option>
	        <option value="3" <s:if test="cmsTemplate.type==3">selected="selected"</s:if>>活动模板</option>
	        <option value="4" <s:if test="cmsTemplate.type==4">selected="selected"</s:if>>资讯模板</option>
	        <option value="5" <s:if test="cmsTemplate.type==5">selected="selected"</s:if>>帮助模板</option>
	        <option value="6" <s:if test="cmsTemplate.type==6">selected="selected"</s:if>>移动端模板</option>
	        <option value="9" <s:if test="cmsTemplate.type==9">selected="selected"</s:if>>商家入驻模板</option>
	        <option value="7" <s:if test="cmsTemplate.type==7">selected="selected"</s:if>>窄版静态专题页模板</option>
	        <option value="8" <s:if test="cmsTemplate.type==8">selected="selected"</s:if>>宽版静态专题页模板</option>
	        <option value="10" <s:if test="cmsTemplate.type==10">selected="selected"</s:if>>广告模板</option>
	        <option value="11" <s:if test="cmsTemplate.type==11">selected="selected"</s:if>>招商帮助模板</option>
	   <!--       <option value="15" <s:if test="cmsTemplate.type==15">selected="selected"</s:if>>抽奖模板</option>
            <option value="16" <s:if test="cmsTemplate.type==16">selected="selected"</s:if>>抽奖窗口模板</option> --> 
            <option value="17" <s:if test="cmsTemplate.type==17">selected="selected"</s:if>>招商入驻</option>
            <option value="18" <s:if test="cmsTemplate.type==18">selected="selected"</s:if>>合作模式</option>
            <option value="19" <s:if test="cmsTemplate.type==19">selected="selected"</s:if>>商家规则</option>
            <option value="20" <s:if test="cmsTemplate.type==20">selected="selected"</s:if>>商家店铺</option>
            <option value="21" <s:if test="cmsTemplate.type==21">selected="selected"</s:if>>活动中心</option>
            <option value="22" <s:if test="cmsTemplate.type==22">selected="selected"</s:if>>商户服务</option>
             <option value="23" <s:if test="cmsTemplate.type==23">selected="selected"</s:if>>活动中心帮助模板</option>
              <option value="24" <s:if test="cmsTemplate.type==24">selected="selected"</s:if>>中药城帮助中心模板</option>
