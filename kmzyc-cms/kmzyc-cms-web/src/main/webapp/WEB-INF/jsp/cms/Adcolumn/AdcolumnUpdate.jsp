...
$(document).ready(function(){
	...
	if(colType != -1){
		 ...
		 $.ajax({
			   ...
			   success: function(data){
			    ...
			    //根据select的显示值来为select设值
			   	var count = 0;
			   	count = $("#podBrandId").get(0).options.length;
			   	if(count == 0){
			   	 	var podBrandId = $("#podBrandId");
			   	 	podBrandId.empty();
				   	var option = $("<option>").text("请选择品牌").val(-1);
			   		podBrandId.append(option);

			   		var output = $("#output").val();
			   		var pId = output.substring(output.lastIndexOf("_")+1,output.indexOf("."));
			   		
			   		for(var i=0;i<objs.length;i++){
				   	  if(pId == objs[i].brandId){
				   		  var option = $("<option selected='selected'>").text(objs[i].brandName).val(objs[i].brandId);	  
					  }else{
						  var option = $("<option>").text(objs[i].brandName).val(objs[i].brandId);
					  }
			   		  podBrandId.append(option);
		 ...
		 $("#podBrand").show();
		 $("#adcolumnType").find("option[text='品牌广告']").attr("selected",true);  
	}
 	$("#AdcolumnUpdate").validate({
               rules: {
 ...
 jQuery.validator.addMethod("compareTo", function(value, element) {  
      var id=$("#adcolumnId").val();
       var ok="";
        $.ajax({
        url:'/cms/cmsPromotion_outPutValidate.action',
        data:{"outPut":value,"outPutType":2,"promotionId":id},
        async:false,
        type:'post',
        success:function (d) {
         if(d=="0"){
 ...
 jQuery.validator.addMethod("compareTo2", function(value, element) {  
       ...
       var ok="";
        ...
        data:{"cmsAdcolumn.name":value,"cmsAdcolumn.adcolumnId":id},
        async:false,
        type:'post',
        success:function (d) {
         if(d=="0"){
...
<s:form  id="AdcolumnUpdate" name="AdcolumnUpdate" action="/cms/Adcolumn_Update.action" method="POST" enctype="multipart/form-data">
<s:token></s:token>
...
<table width="60%"  class="edit_bottom" height="30" border="0" cellpadding="0" cellspacing="0">
	<tr> 
		<td align="left">
            ...
            &nbsp;&nbsp;
			<input class="backBtn"  onclick="gotoList(<s:if test="adminType==0">0</s:if>)" type="button" value="">
		</td> 
