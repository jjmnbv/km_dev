...
<script type="text/javascript">
    ...
    function selectOneAccount(){
    	...
    	parent.closeViewPromotionProduct(checkeds,windowId);
    }
</script>
</head>
<body>
<s:form  name="viewPromotionForm" action="/cms/viewPromotionProductAction_queryViewPromotionProduct.action" onsubmit="return checkAllTextValid(this)" method="post">
<table  width="98%" class="content_table" align="center" cellpadding="0" cellspacing="0"  style="margin:10 0 10 0px;">
	<tr>
		...
		<td><input name="viewPromotion.promotionId" type="text" value="<s:property value='viewPromotion.promotionId'/>"/></td>
		<td align="right">活动标题：</td>
		<td><input name="viewPromotion.promotionTitle" type="text" value="<s:property value='viewPromotion.promotionTitle'/>"/></td>
		  	...
		  	<select name="viewPromotion.promotionTypeId">
		    	<option value="">全部</option>
		     	<s:iterator value="listPromotionType" id="listPromotionType">
		     		<option <s:if test="promotionTypeId == viewPromotion.promotionTypeId">selected="selected"</s:if> value="<s:property value='promotionTypeId'/>">
		     			<s:property value="promotionTypeName"/>
		     		</option>
				</s:iterator>
			</select>
		</td>
		...
		<td>
			<%--<select name="channel">
		    	...
		    	<s:generator val="channelQuery" separator="," id="channels"></s:generator> 
				<s:iterator status="channelIndex" value="#request.channels" id="channelName">
					<option <s:if test="#channelName==channel">selected="selected"</s:if> value="<s:property value="channelName"/>">
	...
	<tr>
		...
		<td align="center"  colspan="4"><input type="submit" class="queryBtn" value=""></td>
	</tr>
</table>
...
<table width="98%" class="list_table" align="center" cellpadding="3" cellspacing="0" border="1" bordercolor="#C1C8D2">
	...
	<tr>
	    <td>
		     <s:if test="flag!=1">
		     	<input type="radio" name="promotionIds" value='<s:property value="promotionId"/>' onclick="checkpromotionIdRadio(this);">
		     </s:if>
	    </td>
		<td align="center">
