...
<html>
              	 ...
              	 if(mychecks==0){
                  	 for(var k = 0;k<checks.length;k++){
                  		 $.ajax({
                  			 ...
                  			 if(myData["isPhy"]==1){
  	       				        	type="物理类目";
  		       				     }else if(myData["isPhy"]==2){
  		       				    	type="运营类目";
  		       				     }
                  			 	 $str='';
         				         $str+="<tr class='dataTr'>";
         				         $str+="<td><input type='checkbox' name='dataIds' value='"+myData["categoryId"]+"' onclick=checkboxChange(); /></td>";
         				      	 $str+="<td>";
	...
	<body onload="loadData();">
		...
		<s:set name="son_name" value="'产品排行榜所需类目数据'" scope="request" />
