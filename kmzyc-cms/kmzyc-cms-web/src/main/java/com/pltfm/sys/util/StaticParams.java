...
public class StaticParams {
	private static Logger logger = LoggerFactory.getLogger(StaticParams.class);
...
/*------------------------------------------  sys_param中的参数 -----------------------------------------------*/
    ...
    private static void loadParamList(String sysCd, String paramGp) {
        ...
        try {
            ...
            SysParamBean bean = SysParamBean.instance();
            paramList = bean.getSysParamList(sysCd, paramGp);
        } catch (Exception e) {
        	logger.error("StaticParams.loadParamList异常：" + e.getMessage(), e);
