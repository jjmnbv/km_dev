...
public class StaticRealPath {
	private static Logger logger = LoggerFactory.getLogger(StaticRealPath.class);
    ...
    public static String getFullPathRelateClass(String relatedPath, Class cls) throws IOException {
        ...
        String tempPath = clsFile.getParent() + "/" + relatedPath;
    ...
    public static String loadWebAppPath() {
        String path = null;
        try {
            path = getFullPathRelateClass("../../../../../../",
                    StaticRealPath.class).replaceAll("%20", " ");
            path = path + "/";
            oddsEventAnalysisWebAppRealPath = path;
            //System.out.println("*************************************oddsEventAnalysisWebAppRealPath="+path);
        } catch (Exception e) {
            logger.error("StaticRealPath.loadWebAppPath异常：" + e.getMessage(), e);
        }
        return path;
    }
