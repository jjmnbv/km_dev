	...
	<body>
	 ...
	 <td>	 				 <s:date name="qualificaitonsFileDO.endDate"  format="yyyy-MM-dd" />
				...
				<table width="60%" class="edit_bottom" height="30" border="0"
					...
					<tr>
						<td align="left">
							<input   type="button"  class="btn-custom" onclick="qualificaitonsFileVerify()"  value="审核通过">
							<input   type="button" class="btn-custom" onclick="qualificaitonsFileNoPass()"  value="审核不通过">
							&nbsp;&nbsp;
...
function qualificaitonsFileVerify()
{ 
	
	var id=$("#id").val();
	alert("审核通过！");
location.href="/qualificaitonsFile/qualificaitonsFile_qualificaitonsFileVerify.action?id="+id+"&status="+2;

}
function qualificaitonsFileNoPass()
{

	var id=$("#id").val();
	alert("审核不通过！");
location.href="/qualificaitonsFile/qualificaitonsFile_qualificaitonsFileNoPass.action?id="+id+"&status="+3;
