	...
	<body>
		...
		<div style="height: 90%; overflow-y: auto;">
			<s:form name="bnesAcctEnchashmentForm" action="/accounts/bnesAcctEnchashment_PageList.action" method="post">
			    <!-- 根据该字段判断是否是提现确认页面  -->
			    <input type="hidden" id="fromConfirmMenu"  value='<s:property value="fromConfirmMenu"/>' />
				<!-- 查询条件 -->
				<table width="98%" align="center" height="120" border="0" class="content_table" cellpadding="0" cellspacing="0">
					<tr>
					...
					<input type="hidden" name="fromConfirmMenu"  value = "no">
					 	审核通过
...
<!-- 						</td> -->
						...
						<td>
						<div align="left">
<!-- 						<input  class="ui-button ui-button-success" type = "button" value="导出excel" onclick="exportDate()"> -->
					...
					<tr>
					<td colspan="2" align="left" style="font-weight:bold;">&nbsp;提现总金额：<fmt:formatNumber value="${totalAmount}" type="currency" pattern="#,##0.00元"/>（仅为审核通过状态下的统计）</td>
					<td colspan="10">&nbsp;</td>	
                    <td><INPUT TYPE="submit" class="queryBtn" value="" onclick = "querySubmit()">
                    <a href="javascript:void(0);" onClick="exportDate()" class="btn-custom" width="40px">导出excel</a></td>
					</tr>
				</table>
				<!-- 数据列表区域 -->
