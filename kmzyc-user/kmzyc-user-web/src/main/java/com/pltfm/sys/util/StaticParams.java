...
public class StaticParams {
  ...
  private static void loadParamList(String sysCd, String paramGp) {
    /*
     * String paraStr = sysCd + "," + paramGp; List paramList = null; try { //System.out.println(
     * "--------------> 第一次查询此参数："+paraStr); // WebApplicationContext context =
     * (WebApplicationContext)this.getServletContext().getAttribute(WebApplicationContext.
     * ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE); // SysParamService sysParamService =
     * (SysParamService)context.getBean("sysParamService"); ApplicationContext context = new
     * ClassPathXmlApplicationContext(new String[] {"spring-context-application.xml"}); // an
     * ApplicationContext is also a BeanFactory (via inheritance) BeanFactory factory =
     * (BeanFactory) context; SysParamService sysParamService =
     * (SysParamService)factory.getBean("sysParamService"); paramList =
     * sysParamService.getSysParamList(sysCd,paramGp); } catch (Exception e) { e.printStackTrace();
     * //System.out.println("[-FAIL->> 参数:" + sysCd + paramGp + "列表获取失败!\n" + e.getMessage()); } if
     * (null != paramList) { paramMap.put(paraStr, paramList); }
     */
  }
