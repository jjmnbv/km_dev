  ...
  public String doUpload() throws Exception {
    if (doc != null && doc.length != 0) { // 如果文件上传不为空
          ...
          if (relativePathArray[i] != null && !relativePathArray[i].equals("")) {
            ...
            if (!indexFile.exists()) {
              String indexFileContent =
                  "<script>location.href='<%=request.getContextPath()%>/index.html';</script>";
      ...
      for (int i = 0; i < doc.length; i++) {
        ...
        // 将图片进行物理上传
        SysUploadFile fm = FileUploadUtil.oneUpload(absolutePath, relaPathStr, fileName[i],
            contentType[i], doc[i], i);
        ...
        sysUploadFileService.addUploadFileInfo(fm);
