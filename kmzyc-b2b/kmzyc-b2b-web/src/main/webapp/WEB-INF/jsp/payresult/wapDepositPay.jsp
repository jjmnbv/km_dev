							...
							<s:if test="isWxPay == 0">
								...
								<li class="none">
			            			<label class="defray-logo">
				              			<input type="radio" class="payment" name="payment"  value="7">
				              			<span class="defray-logo-img" class="pay-btn" title="康美通支付">
				                  			<img alt="康美通支付" src="<%=wapCssPath%>/reswap/images/template/kmtpay.png" />
				              			</span> 
			            			</label>
			            			<c:if test="<%=Calendar.getInstance().get(Calendar.MONTH)==4&&Calendar.getInstance().get(Calendar.DATE)>17&&Calendar.getInstance().get(Calendar.DATE)<29%>">
		              			<span class="recommended">
					                <i class="icon-recommended"><img src="<%=wapCssPath%>/reswap/images/tips.png"></i>
					                <p style="margin-left: 10px;">首次使用满60元立减15元；老用户满60元立减5元</p>
					            </span> 
                               </c:if>
								</ul>	
							</s:if>
