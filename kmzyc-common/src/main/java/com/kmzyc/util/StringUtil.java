...
public class StringUtil {
  ...
  private static final Pattern mobilePattern = Pattern.compile("^1[3|4|5|6|7|8]\\d{9}$");// 手机格式

  private static final Pattern timeUserPattern = Pattern.compile("^[a-zA-Z]\\d{5,19}$");// 时代帐号格式

  private static final DateTimeFormatter simpleJoin = DateTimeFormatter.ofPattern("yyyyMMddHHmmss");
  ...
  public static boolean isInArray(int value, String[] array) {
    if (array == null) return false;
    for (String v : array) {
      if (Integer.parseInt(v) == value) return true;
    }
    return false;
  }
...
//  }
  ...
  public static String htmlDecode(String htmlContent) {
    ...
    htmlContent =
        htmlContent.replaceAll(" ", "&nbsp;").replaceAll("<", "&lt;").replaceAll(">", "&gt;")
            .replaceAll("\n\r", "<br>").replaceAll("\r\n", "<br>").replaceAll("\r", "<br>");
    return htmlContent;
  }
   ...
   * @return
   */
  public static boolean withinRange(String str, int min, int max) {
    return null != str && str.length() >= min && str.length() <= max;
  }
   ...
   * @return
   */
  public static boolean equalLen(String str, int len) {
    return null != str && str.length() == len;
  }
   ...
   * @return
   */
  public static boolean withinRangeByte(String str, int min, int max) {
    if (null != str) {
      try {
   ...
   * @return
   */
  public static boolean equalLenByte(String str, int len) {
    if (null != str) {
      try {
        return len == str.getBytes("GBK").length;
      } catch (Exception e) {}
    }
    return false;
  }
   ...
   * @return
   */
  public static String getSimpleJoin(Date date) {
    return null == date ? null : simpleJoin.format(date.toInstant());
  }
  ...
  public static boolean isMobile(String mobile) {
    return null != mobile && mobilePattern.matcher(mobile).matches();
  }
   ...
   * @return
   */
  public static boolean isTimesUName(String userName) {
    return null != userName && timeUserPattern.matcher(userName).matches();
  }
   ...
   * 生成 len 以内的随机数
   * @param len
   * @return 随机数字符，左边不够 len - 1 的长度 补0
   */
  public static String getRandom(int len){
    ...
    int l = (String.valueOf(len - 1)).toCharArray().length - (String.valueOf(t)).toCharArray().length;
    if(l > 0){
      return String.valueOf(temp.substring(0,l) + t);
    }else{
   ...
   * @return
   */
  public static String getLoginAccount(int type){
    String loginAccount = "";
    if(Constants.CUSTOMER_TYPE_SD_MEMBER == type){
      loginAccount = Constants.LOGACCOUNT_NAME_PREFIX + System.currentTimeMillis() + StringUtil.getRandom(10000);
    }
    return loginAccount;
  }
   ...
   * @return
   */
  public static String passwordTwiceEncryption(String password,String saltStr){
    return MD5.getMD5Str(password+saltStr).toLowerCase();
  }

  /**
   * 生成密码二次加密串
   * @param password
   * @return saltStr 加密串  twicePassword 加密后的字符串
   */
  public static Map<String, String> generatePasswordTwiceEncryption(String password){
    Map<String,String> map = new HashMap<>();
    String saltStr =getRandom(600000);
    map.put("saltStr",saltStr);
    map.put("twicePassword",MD5.getMD5Str(password+saltStr).toLowerCase());
    return map;
  }
   ...
   * @return
   */
  public static String toNormalStr(String string){
    return toEnglish(toSemiangle(string));
  }
   ...
   * @return
   */
  public static List transformStrToList(String string,String split){
    List<String> payRangeList = new ArrayList<>();
