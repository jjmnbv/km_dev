...
//20.进入配送信息导入页面
function gotoImportDisInfoPage() {
	
	window.location.href = "/app/gotoImportDisInfoPage.action";
}

//21.执行配送信息导入
function doImportDisInfo() {
	var importDisFile = $("#importDisFile").val();
	if(importDisFile == '' || importDisFile == null){
		alert("请先选择导入文件再操作！");
		return false;
	}
	var suffix = importDisFile.substr(importDisFile.lastIndexOf('.'),importDisFile.length);
	if(suffix != '.xls'){
		alert("导入文件格式错误，目前只支持.xls的excel格式文件！");
		return false;
	}
	
	$.ajaxFileUpload({
       url:'/app/doImportDisInfo.action',//用于文件上传的服务器端请求地址
       data:$("#importDisFrm").serializeArray(),
	   secureuri:false,//一般设置为false
	   fileElementId:['importDisFile'],//文件上传空间的id属性  <input type="file" id="file" name="file" />
	   dataType: 'json',//返回值类型 一般设置为json
       success: function (data){
    	   $("#importResult").html(data.msg);
       }
	});
}
...
function showFileInfo() {
	...
	$("#show_file_path").val($("#importDisFile").val());
}
