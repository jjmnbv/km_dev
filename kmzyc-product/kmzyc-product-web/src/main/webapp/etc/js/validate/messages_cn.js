...
//不允许输入全角字符（包括汉字）//只能输入英文字母和半角字符
jQuery.validator.addMethod("checkSbc", function(value, element) {
    var sbcTest = /[^\x00-\xff]/;
    return this.optional(element) || !sbcTest.test(value);
}, "不允许输入全角字符（包括汉字）");

//中文大写拼音
jQuery.validator.addMethod("checkChinesePinYin", function(value, element) {
    var tel =/^[A-Z\s]+$/;
    return this.optional(element) || (tel.test(value));
}, "只能输入大写英文字母和空格");
