     ...
     * @param value 产品id
     */
    function illegalDown(id){
        if(confirm('确定将此产品违规下架吗？')){
            var dia = art.dialog({
                title:'请填写产品违规下架的原因',
                content: $("#reason").html(),
                drag:true,
                ...
                ok: function () {
                    ...
                    $.ajax({
                        type: "POST",
                        dataType:'json',
                        url:'/app/illegalDown.action',
                        data: {"product.productId": id, "product.reasons": reason},
                        error:function(){alert('请求失败，请稍后重试或与管理员联系！')},
     ...
     * @param value 产品id
     */
    function illegalUp(value){
        if(confirm('确定将此违规下架的产品重新上架吗？')){
            $.ajax({
                type: "POST",
                dataType:'json',
                url:'/app/illegalUp.action',
...
//			}
    ...
    function gotoProductOperation(id){
        $('#frm').attr("action","/app/showOperation.action?productId="+id);
        $('#frm').submit();
    }
	...
	function gotoListForView(){
		if($("#returnType").val()!="" && $("#returnType").val()!='recheck' && $("#returnType").val()!='illegal'){
			$('#listfrm').attr("action", "/app/findCertificateFilesProduct.action");
		}
	
		//20151221 add  回到返利网超级返页面
		if($("#returnType").val().indexOf("fanliProduct")>-1){
			$('#listfrm').attr("action", "/fanli/queryFanLiProductList.action?querySource=menu");
		}
...
//			$("#img_"+productId).attr("src","/etc/images/little_icon/huifu.png");
			...
			$.post(
				...
				function(data){
					...
					$("#img_"+productId).attr("src","/etc/images/little_icon/tips.png");
