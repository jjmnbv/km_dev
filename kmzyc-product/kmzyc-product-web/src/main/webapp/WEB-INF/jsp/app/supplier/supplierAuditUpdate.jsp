...
<s:form action="supplierUpdate.action" method="POST"
    ...
    <table width="95%" class="edit_table" align="center" cellpadding="3"
        ...
        <s:if test="rtnMessage != null">
            <tr>
                <td colspan="2" align="center"><font color="red"><s:property value='rtnMessage' /> </font>
                </td>
            </tr>
        </s:if>
        <tr>
        ...
        <tr>
            ...
            <td>
                <s:if test="suppliersInfo.supplierType == 4">
                    <input type="hidden" value="4" id="suppliersTypeId" name="suppliersInfo.supplierType"/>康美中药城
                </s:if>
                <s:else>
                    <s:select list="#{2:'入驻',3:'代销'}" name="suppliersInfo.supplierType"
                              headerKey="" id="suppliersTypeId" headerValue="--请选择--"></s:select>
                </s:else>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                <select name="merchantOrSupplier.province" id="province"
                        data-value="<s:property value="merchantOrSupplier.province"/>">
                </select>
                <select name="merchantOrSupplier.city" id="city"
                        data-value="<s:property value="merchantOrSupplier.city"/>">
                </select>
                <select name="merchantOrSupplier.area" id="area"
                        data-value="<s:property value="merchantOrSupplier.area"/>">
                </select>
        ...
        <tr>
            <th align="right" class="eidt_rowTitle"><font color="red">*</font>分配经营类目(比例为百分比)：</th>
            <td class="Supplierslist">
                <input type="hidden" name="categoryValueList" id="categoryValue">
                <input type="hidden" name="categoryValueList1" id="categoryValue1">
                ...
                <s:iterator value="categorysList" id="hisList1">
                    <s:if test="#hisList1.categoryParentId == 0">
                        <input type="hidden" style="width: 50px;" value="${sacId}" id="hid${categoryId}"/>
                        <input type="hidden" value="${categoryId}"  class="hidSacId"/>
                        <input type="checkbox" id="box${categoryId}" value="${categoryId}"
                               <s:if test='#hisList1.sacId != null'>checked="checked"</s:if>
                               onclick="checkedInput('${categoryId}')" name="categorys"
                               class="ui-form-checkbox class${categoryParentId}"/>
                        ${categoryName}
                            ...
                            <s:iterator value="categorysList" id="hisList2">
                                <s:if test="#hisList2.categoryParentId != 0">
                                    <s:if test="#hisList1.categoryId == #hisList2.categoryParentId">
                                        ...
                                        <input type="hidden" value="${categoryId}"  class="hidSacId"/>
                                        <input type="checkbox" id="box${categoryId}" value="${categoryId}"
                                               <s:if test='#hisList2.sacId != null'>checked="checked"</s:if>
                                               onclick="checkedInput('${categoryId}')" name="categorys"
                                               class="ui-form-checkbox class${categoryParentId}"/>
                                        ${categoryName}
                                        <input type="text" name="inputRat${categoryParentId}" class="inputRat"
                                               style="width: 50px;" id="inputs${categoryId}"
                                               <s:if test='#hisList2.sacId == null'>disabled="true"</s:if>
                                               maxlength="5" value='${commissionRatio}' title="请输入佣金比例"/>%
                                    </s:if>
                                </s:if>
                            </s:iterator>
                        </li>
        ...
        <tr>
            ...
            <td>
                <input type="hidden" value="${suppliersWarehouse.supWarehouseId}"
                       name="suppliersWarehouse.supWarehouseId"/>
                <s:select list="#request.warehouseInfoStatusMap"
                          value="suppliersWarehouse.warehouseId" id="warehouseId"
                          name="suppliersWarehouse.warehouseId" headerKey="" headerValue="--请选择仓库--"></s:select>
            </td>
        </tr>
        <tr>
                ...
                <ul id="addFile" >
                    <s:iterator value="suppliersCertificateList">
                        <li id="hrefId${scId}" style="float: left;list-style-type:none;width: 200px;height:220px;">&nbsp;&nbsp;
                            <a href="${imagePath}${filePath}" title="单击放大" target="_blank" id="hrefId${scId}">
                                <img width="190px" height="190px" src="${imagePath}${filePath}"/>${fileName}</a>&nbsp;&nbsp;
                            <a href="javascript:void(0);" id="del${scId}" onclick="deleteFiles(${scId})">删除</a>
                        </li>
                    </s:iterator>
                </ul>
            </td>
    ...
    <input type="hidden" name="merchantOrSupplier.merchantId" id="merchantIdVal"
           value="<s:property value='merchantOrSupplier.merchantId' />"/>
    <input type="hidden" name="suppliersInfo.supplierId" id="supplierIdUp"
           value="<s:property value='suppliersInfo.supplierId' />"/>
    ...
    <input type="hidden" name="selectSuppliersInfo.supplierType"
           value="<s:property value='selectSuppliersInfo.supplierType' />"/>
</s:form>
        ...
        <td>
            <s:form action="uploadCeriticate.action" namespace="app" name="frm1" method="post" enctype="multipart/form-data" id="frm1" class="ui-form ui-form-vertical fn-t20" >
                ...
                <div class="ui-form-item">
                    ...
                    <input type="text" tabindex="9" id="fileName" name="realName"
                           class="ui-input fn-w280 fn-mr10" placeholder="请输入文件名称" />
                    <input tabindex="10" id="upFile" contenteditable="false" class="fn-w180 fn-mr20"
                           name="file" type="file" />
                    <span id="error_message" class="fn-red"></span>
                    <input type="button" value="上传文件" onclick="submitForms();"/>
...
<table width="98%" align="center" class="edit_bottom" height="30"
    ...
    <tr>
        <td align="center">
            <input class="saveBtn" type="button" onclick="goSubmit()"/> &nbsp;&nbsp;
            <input class="backBtn" type="button" onclick="gotoList()" />
...
<script type="text/javascript">
                    ...
                    for (var i = 0; i < cate.length; i++) {
                        ...
                        if (cate[i].checked == true && isTrue) {
                            ...
                            if (strClass[1] != 'class0') {
                                ...
                                if (cate[i].checked && !reg.test(ckbili
                                                || ckbili.substring(ckbili.indexOf(".") + 1, ckbili.length).length > 2
                                                || ckbili == 0)) {
    ...
    function ckboxCretegory() {
        var ckTypesVal = "0";
        var ckTypesVa2 = "0";
        var categoryNameTag = document.getElementsByName("categorys");
        var str = "";//保存要新增的渠道类型名称
        var bili = "";//保存佣金比例
        var sacIdStr = "";//id
        var str1 = "";
        if ("2" == $("#suppliersTypeId").val()) {
            for (var i = 0; i < categoryNameTag.length; i++) {
                if (categoryNameTag[i].checked) {
                    var categoryId = categoryNameTag[i].value;
                    str += categoryId + ",";
                    bili += $("#inputs" + categoryId).val() + ",";
                    sacIdStr += $("#hid" + categoryId).val() + ",";
                    ckTypesVal = "1";
                    ckTypes1 = true;
                }
            }
    ...
    function checkedInput(val) {
        var checkbox = document.getElementById("box" + val);
        var strClass = (checkbox.className).split(' ');
        var nodeId = '';//父节点id
