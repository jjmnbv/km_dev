...
<s:set name="name" value="'追加评价审核'" scope="request" />
<jsp:include page="/WEB-INF/jsp/common/title.jsp"></jsp:include>

<s:form action="/app/showAddtoContentList.action" method="POST"  namespace='/app' id="frm" name='frm'>
...
<table id="mytable" width="98%" class="list_table"  cellpadding="3" align="center" cellspacing="0" border="1">
	<tr>
		...
		<th align="center" width="70%">追加内容</th>
	...
	<tr>
	    <td align="center" width="5%">
	    	<s:if test='checkStatus != 1'>
	    		<input type="checkbox" name="delId"  value='<s:property value="addContentId"/>'>
	    		<input name="checkStatus" type="hidden" value="<s:property value='checkStatus'/>" />
	    	</s:if>
		    <s:else>
		    	&nbsp;
		    </s:else>
		...
		<td align="center">
			<span title="<s:property value="addContent"/>" style="width:800px;height:31px;overflow:hidden;display:block;line-height:31px;"><s:property value="addContent"/></span>
		</td>
		<td align="center">
			<s:date name="addContentDate" format="yyyy-MM-dd" />
		</td>
		<td align="center">
...
function doDelete(name){
	document.forms['frm'].action="/app/deleteAddtoContent.action";
...
function gotoAudit(arg){
	...
	if(confirm("确定执行该操作吗？")){
		document.forms['frm'].action="/app/addToContentToAudit.action?statusArg="+arg;
		document.forms['frm'].submit();
	}
}
