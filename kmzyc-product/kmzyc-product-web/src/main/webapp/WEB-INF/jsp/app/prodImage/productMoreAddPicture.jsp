...
<html>
	...
	$(document).ready(function() {
		...
		$('#btnSave').click(function(){
			...
			window.location = '/app/productShow.action';
		});
     ...
     function uploadpreview(serverData) { 
		...
		_input.name = "imageId";
     	...
     	if(upnum == 1){
     		...
     		str = "<span>&nbsp;<img src='/etc/images/first.gif' alt='默认' title='默认' id='rpic_"+picId+"' onclick='javascript:setMainPic("+productSkuId+","+picId+","+index+",this.parentNode.parentNode.parentNode);' /></span>";
     		str += "<span class=''>&nbsp;<img src='/etc/images/del16.gif' alt='删除' title='删除' onclick='delPic(this.parentNode.parentNode.parentNode,\""+data[0]+"\","+productSkuId+","+index+",\""+data[3]+"\")' /></span>";
     		...
     		$.post(
   				...
   				function(data){
   					if(data=="success"){
   						$("input[id='rpic_"+picId+"']").attr("checked",true);
   						oldRadio[index] = "rpic_"+picId;
   					}
   				}
    		);
     	}else{
	 ...
	 function ajaxsetMainPic(productSkuId,picId,index){
		 $.post(
			...
			function(data){
				if(data=="success"){
					$("#uploadpreview_"+index).find("div.preview:first").addClass("isDefault").siblings().removeClass("isDefault");
				}
			}
		);
	 }
	...
	border-width: 2px;
 	...
 	<div class="container">
 		...
 		<div class="adbt" style="color:black;">
	 		...
	 		<p id="uploadTip">产品名称：<b><s:property value="product.name" /></b></p>
 		</div>
 		<s:iterator value="#request.productSkuList" var="data" status="st" >
 		<input id="productSkuId<s:property value='#st.index' />" type="hidden" value="<s:property value="#data.key.productSkuId" />" />
			...
			<tr>
				<td>
					<div class="userprofile batchupload"  style="float:left;width:270px;clear: both;">
						<!--upload-->
						<fieldset>
							SKU编码：<b><s:property value="#data.key.productSkuCode" /></b>
							<p style="font-size:14px;">
								<s:if test='#data.value.size() == 0'>
									无SKU规格
								</s:if>
								<s:else>
									<s:iterator value="#data.value" >
				...
				<td>
					<form action="/basedata/updateImageSortNo.action" method="post" name="imageFrm" id="imageFrm_<s:property value='#st.index' />" target="theID" >
