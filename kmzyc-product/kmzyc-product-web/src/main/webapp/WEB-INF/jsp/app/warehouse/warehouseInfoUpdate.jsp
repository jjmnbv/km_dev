...
<s:set name="son_name" value="'仓库修改'" scope="request"/>
<jsp:include page="/WEB-INF/jsp/common/title.jsp"></jsp:include>

<s:form action="/app/warehouseUpdate.action" method="POST" namespace='/app'  id="wrm" name="wrm" >
	<s:hidden name="warehouseInfo.warehouseId" id="warehouseIdHidden"/>
	<!-- 数据编辑区域 -->
	<table width="95%" class="edit_table" align="center" cellpadding="3"
		...
		<tr>
		  ...
		  <td width="80%">
		  	<s:select list="#request.areaList" id="areaId1" name="areaId1" listKey="areaId" listValue="areaName" headerKey="" headerValue="--请选择省份--"
		  		onchange="choiceArea('areaId1','areaId2')"></s:select>
		  	<select id="areaId2" name="warehouseInfo.areaId" onchange="choiceCity();">
		    	<option value="${warehouseInfo.areaId}">${warehouseInfo.areaName}</option>
            </select>
		...
		<tr>
			...
			<td><label> <s:textfield name="warehouseInfo.warehouseName"
				id="warehouseName" size="32" maxlength="32" onblur="checkWarehouseNameByModify()"/> </label></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>
			<s:textfield name="warehouseInfo.overlayArea" id="overlayArea" size="32" maxlength="512" readonly="true" cssStyle="background:#BEBEBE"/>
		...
		<%--<tr>
			...
			<s:hidden name="warehouseInfo.warehouseRelation.warehouseRelationId"></s:hidden>
		</tr>
		<tr>
		  <th align="right" class="eidt_rowTitle">外部系统名称：</th>
		  <td><s:select list="#request.systemCodeMap" name="warehouseInfo.warehouseRelation.systemCode" id="systemCode"></s:select></td>
	  	</tr>
		<tr>
		  <th align="right" class="eidt_rowTitle">外部系统仓库编码：</th>
		  <td><s:textfield name="warehouseInfo.warehouseRelation.externalWarehouseCode" id="externalWarehouseCode" size="32" maxlength="32" /></td>
	  	</tr>
		<tr>
