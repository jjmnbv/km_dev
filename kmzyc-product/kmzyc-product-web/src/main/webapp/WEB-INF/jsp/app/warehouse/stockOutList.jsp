...
<s:set name="name" value="'出库单列表'" scope="request" />
<jsp:include page="/WEB-INF/jsp/common/title.jsp"></jsp:include>
	
<s:form action="findStockOutList" method="post"  namespace='/app' id="stockOutfrm" 
...
<table  width="98%" class="content_table" align="center" cellpadding="0" cellspacing="0" >		
	...
	<tr>
	   <td>状态:<s:select list="#request.stockOutStatusMap" name="queryStockOut.status"  id="stockOutStatus" headerKey="" headerValue="--全部状态--"></s:select>
      </td>
       <td >
    ...
    <tr> 
     <td >
	          类型:<s:select list="#request.stockOutTypeMap" name="queryStockOut.type"  id="stockOuttype" headerKey="" headerValue="--全部类型--"></s:select>
	</td>
	<td>
	...
	<td>
	  ...
	  <td><input class="queryBtn" type="button" onClick="searchStockOut();"  />	 <input class="addBtn" type="button"  onclick="toAddStockOut();" />
		  <input class="delBtn" type="button" onClick="deleteUnCheckedStockOut();"  /></td>
	</tr>
...
<table width="98%" class="list_table" align="center" cellpadding="3" cellspacing="0" border="1" bordercolor="#C1C8D2">
		...
		<s:iterator id="pdata" value="page.dataList" >
			<tr>
		   	  ...
		   	  <s:elseif test="status!=0" ><td align="center" width="3%"><input type="checkbox" name="stockOutChk" disabled="true"  value='<s:property value="stockOutId"/>'></td></s:elseif>
				...
				<td align="center">
					<img title="详情" style="cursor: pointer;" src="/etc/images/view.png"  onclick="gotoStockOutDetail(<s:property value='stockOutId'/>)" />
				</td>
			</tr>
		</s:iterator>
