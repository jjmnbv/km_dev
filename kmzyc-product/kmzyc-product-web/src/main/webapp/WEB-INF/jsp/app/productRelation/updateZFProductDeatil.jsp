...
<head>
    ...
    <script type="text/javascript" src="/etc/js/warehouse/distributionInfo.js"/>
...
<div style="position: absolute; align: center; top: 20px; left: 30px">
    ...
    <s:form action="/productRelation/updateProductZFRelation.action"
            method="post" namespace='/productRelation' id="relationfrm" name='relationfrm'>
        <input type="hidden" name="oldRIdCorrelationSIds" value=""/>
        <input type="hidden" name="productRelation.relationId"
               value="<s:property value='productRelationDetail.relationId'/>"/>
        <input type="hidden" id="mainSkuIds" name="mainSkuIds" value=""/>
        <div id="content">
            <div>
                <table width="90%" class="table_search" align="center" cellpadding="0"
                    ...
                    <tr>
                        ...
                        <td width="432px">主产品价格：&nbsp;&nbsp;&nbsp;<s:property
                                value="productRelation.mainSkuPrice"/></td>
                        <td>组方名称：<s:textfield name="productRelation.relationName" cssClass="input_style"
                                              reg="^(\S{0,10})$" tip="名称不要超过10个汉字" id="productName"/>
                        </td>
                    </tr>
                    <!-- 组方关联类型：4 -->
                ...
                <table width="98%" class="list_table" align="center" cellpadding="3"
                    ...
                    <s:iterator id="productiterator" value="page.dataList" status="index">
                        <tr>
                            <td align="center">
                                <input type="checkbox" name="relationDetailId" value="<s:property value='skuId'/>"/>
                            </td>
                            ...
                            <td align="center" style="word-break: break-all"><s:property
                                    value="newPrice"/></td>
                            <td align="center" style="word-break: break-all">
                                <img title="删除" style="cursor: pointer; text-align: right"
                                     src="/etc/images/little_icon/delete.png" class="skuIds<s:property value='skuId'/>"
                                     onClick="delOp(<s:property value='skuId'/>)"/></td>
                        </tr>
                    </s:iterator>
                </table>
                <br/>
            ...
            <div>
                组方简介：
                <s:textfield name="productRelation.relationIntro" cssClass="video_intro"
                             reg="^(\S{0,10})$" tip="简介不要超过10个汉字" id="relationIntro"></s:textfield>
            </div>
        </div>
        <br/>
        <table>
...
<s:form action="/app/saveSectionsDetail.action" method="POST" id="saveForm">
    <s:iterator id="productiterator" value="page.dataList" status="index">
        <input type="hidden" id="skuIds<s:property value="skuId" />" name="skuIds"
               value="<s:property value="skuId" />"/>
        <input type="hidden" name="oldRIdCorrelationSId"
               value="<s:property value="skuId" />_<s:property value="relationDetailId" />"/>
    </s:iterator>
</s:form>
...
<script type="text/javascript">
    ...
    function selectList(theButton) {
        ...
        boxlist.each(function (i) {
            ...
            $(this).parent().parent()
                    .find("input[name='productRelationSku']")
    ...
    function addProduct() {
        ...
        if (relationType == 0) {
            var url = "/productRelation/addOtherTypeProductRelation.action?productTied.productSkuId=" + mainSkuId +
                    "&relationType=" + relationType + "&pageNum=" + pageNum +
                    "&checkedId=" + checkedId;
            ...
            var url = "/productRelation/addOtherTypeProductRelation.action?productTied.productSkuId=" + mainSkuId + "&relationType=" + relationType + "&pageNum=" + pageNum + "&checkedId=" + checkedId;
            myDialog = art.dialog.open(url, {
                title: '添加关联产品',
                width: 900,
                height: 500,
    ...
    function saveList() {
        var oldRIdCorrelationSIds = "";
        var strSkuIds = document.getElementsByName("skuIds");
        var strSkuIdsObjarray = strSkuIds.length;
        //获取要删除rIds
        var strRSIds = document.getElementsByName("oldRIdCorrelationSId");
        var strRSIdsObjarray = strRSIds.length;
        var isInclude;
        var isTrue;
        if (strSkuIdsObjarray > 0) {
            for (var i = 0; i < strRSIdsObjarray; i++) {//数据库原有的skuId和rId
                isTrue = true;
                isInclude = strRSIds[i].value.split("_");
                for (var j = 0; j < strSkuIdsObjarray; j++) {//所有存在的skuid
                    if (isInclude[0] == strSkuIds[j].value) {//判断是否存在于相同的skuId
                        isTrue = false;
                        break;
                    }
                }
                if (isTrue) {//不存在则记录数据库已存在rId用作删除动作
                    oldRIdCorrelationSIds += isInclude[1] + ',';
                }
            }
            $(':input[name="oldRIdCorrelationSIds"]').val(oldRIdCorrelationSIds);
        } else {
            alert('请添加关联产品！');
            return;
        }
