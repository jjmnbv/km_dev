...
<html>
	...
	$(document).ready(function() {
		swfupload = new SWFUpload({
			// Backend Settings
			upload_url: "/basedata/uploadProductImage.do",	// Relative to the SWF file (or you can use absolute paths)

		
			file_post_name: "file",
			// File Upload Settings
			file_size_limit : "1024",	// 4MB
		...
		$('#btnBack').click(function(){
			...
			if($("#backPage").val() == "updatePage"){
				window.location = "/app/findProductSkuByProductId.action?productId="+$("#productId").val();
			}else{
				$("#backFrom").submit();
			}
		});
		
		checkRadioId = $("#checkRadioId").val();
     });
     ...
     function uploadpreview(serverData) { 
		...
		_input.name = "imageId";
     	...
     	if(upnum == 1){
     		str = "<span>&nbsp;<img src='/etc/images/first.gif' alt='默认' title='默认' id='rpic_"+picId+"' onclick='javascript:setMainPic("+productSkuId+","+picId+",this.parentNode.parentNode.parentNode);'></span>";
     		str += "<span class=''>&nbsp;<img src='/etc/images/del16.gif' alt='删除' title='删除' onclick='delPic(this.parentNode.parentNode.parentNode,\""+data[0]+"\","+productSkuId+")' /></span>";
     		str += "&nbsp;<img src='/etc/images/up.png' alt='提前' title='提前' onclick='goForward(this.parentNode.parentNode);' />";
     		str += "&nbsp;<img src='/etc/images/down.png' alt='后退' title='后退' onclick='backUp(this.parentNode.parentNode);' />";
...
.isDefault{
			...
			<!--upload-->
				<fieldset>
					...
					<p style="font-size:14px;">
						...
						<input id="productId" type="hidden" value="<s:property value="viewProductSku.productId" />" />
					</p>
					<p style="font-size:14px;">
				...
				<p class="btm"><span id="spanButtonPlaceholder"></span><button id="btnStartUpload" class="table_btn" onclick="javascript:flashUploadStart();" disabled="disabled">点击上传</button><button id="btnBack" class="table_btn">返回</button><button id="btnCancel" class="table_btn" onclick="cancelQueue(swfupload);" style="visibility: hidden;" disabled="disabled">取消</button></p>
			</div>
			<form action="/basedata/updateImageSortNo.action" method="post" name="imageFrm" id="imageFrm" target="theID" >
				...
				<s:iterator value="#request.imageList">		 
					<div id="pic_<s:property value="imageId" />" class="preview<s:if test='isDefault == 0'> isDefault</s:if>" >
						...
						<p class="editDataDiv" style="display: none;" >
							...
							<span>
								&nbsp;
								<img src="/etc/images/first.gif" alt="默认" title="默认" onclick='javascript:setMainPic(<s:property value="skuId" />,<s:property value="imageId" />,this.parentNode.parentNode.parentNode);' />
								<%--
							...
							<span class=''>
								&nbsp;
								<img src="/etc/images/del16.gif" alt="删除" title="删除" onclick='delPic(this.parentNode.parentNode.parentNode,<s:property value="imageId" />,<s:property value="skuId" />)' />
								<%--
							...
							&nbsp;
							<img src="/etc/images/up.png" alt="提前" title="提前" onclick="goForward(this.parentNode.parentNode);" />
							&nbsp;
							<img src="/etc/images/down.png" alt="后退" title="后退" onclick="backUp(this.parentNode.parentNode);" />
						</p>
					</div>
				</s:iterator>
			</div>
	...
	<s:form action="/basedata/findAllSkuProduct.action" namespace="basedata" method="post" id="backFrom">
		<s:hidden name="viewProductSkuCondition.productNo"/>
		<s:hidden name="viewProductSkuCondition.procuctName"/>
		<s:hidden name="viewProductSkuCondition.productSkuCode"/>
		<s:hidden name="viewProductSkuCondition.status"/>
