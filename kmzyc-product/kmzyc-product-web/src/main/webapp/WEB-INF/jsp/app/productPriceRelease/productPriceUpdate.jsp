...
<s:form action="/app/updateProductSKUPrice.action" method="POST"
	namespace="/app" id="frm" onsubmit="return checkPrice();" >
    <s:token></s:token>
	<input type="hidden" name="productId" value="<s:property value="productId" />"/>
	<input type="hidden" name="productNo" value="<s:property value="productNo" />" />
...
<!-- 数据编辑区域 -->
<table width="98%" class="edit_table" align="center" cellpadding="3" cellspacing="0" border="1" bordercolor="#C7D3E2"
       style="border-collapse: collapse;font-size:12px;margin-left:12px;float:left;">
	<tr> 
	...
	<s:iterator value="#request.productSkuList" >
		<tr> 
			<td align="left">
				...
				<input type="hidden" id="skuCostPrice<s:property value="productSkuId" />" name="skuCostPrice" value="<s:property value="skuCostPrice"/>" />
			...
			<td align="right" class="<s:property value="productSkuId" />" ><s:if test='skuCostPrice != null'><s:property value="%{getText('{0,number,##.##}',{skuCostPrice})}" /></s:if><s:else>暂无价格</s:else></td>
			...
			<s:if test='type == "update"'>
				<td align="center">
					<input type="button" class="btnStyle" value="修 改"
                           onclick="priceChange(this,<s:property value="productSkuId" />)" />
				</td>
			</s:if>
		</tr>
	</s:iterator>
...
<s:form action="/app/productPriceShow.action" method="POST" namespace='/app' id="listfrm" name='listfrm'>
	...
	<s:hidden name="type" value="price"></s:hidden>
</s:form>
</body>
<script language="JavaScript">
    ...
    function priceChange(btn, skuId) {
		if(btn.value=='修 改'){
			...
			btn.value = '确 定';
			setTimeout(function(){$(".modify"+skuId)[0].select();},10);
		}else{
			var price = "";
			var costPrice = "";
			var markPrice = "";
			...
			if(!havePass){
				alert(strMsg);
				setTimeout(function(){_input.select();},10);
				return;
			}
            if (price == '') {
                price = "0";
