...
<s:form action="/supplierActivity/findChannelSupplierProductSalesList.action" method="POST" id="frm" name='frm'>
	<!-- 查询条件区域 -->
			...
			<tr>
				...
				<td align="center">
					...
					<s:if test="appendPreSaleQuantity>0">
						<a class="link" tabindex="0" style="cursor: pointer;" 
							onclick="queryAppendProductDetail(
							<s:property value='activitySkuParam.activityId' />,
							<s:property value='activitySkuParam.supplierEntryId' />,
							<s:property value='productSkuId' />,
							<s:property value='activityType' />);"
						>
							<span style="font-family: '应用字体 Regular', 应用字体; text-decoration: underline; color: rgb(0, 102, 153);">（已追加<s:property value='appendPreSaleQuantity' />）</span>
						</a>
					</s:if>
				</td>
				<td align="center">
				...
				<td align="center">
					<img title="查看销售明细" style="cursor: pointer;"
						 ...
						 onclick="querySkuSalesDetail(<s:property value='activitySkuParam.activityId' />,<s:property value='supplierEntryId' />,<s:property value='productSkuId' />,<s:property value='activityType' />)" />
				</td>
			</tr>
		</s:iterator>
	</table>
