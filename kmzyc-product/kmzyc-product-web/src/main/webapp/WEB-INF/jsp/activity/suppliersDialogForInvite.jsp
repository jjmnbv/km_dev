...
<script type="text/javascript" src="/etc/js/activity/suppliersDialogForInvite.js"></script>
<style type="text/css">
body {
	padding: 0px;
	margin: 0px;
...
<s:form action="/supplierActivity/selectSupplierForInvite.action"
	method="post" namespace='/supplierActivity' id="frm" name='frm'>
		<s:hidden name="checkedId" id="checkedId"/>
		<input type="hidden" name="activityInfo.activityId" value="<s:property value='#request.activityId'/>"/>
		<input type="hidden" id="inviteSuppliers" value="<s:property value='#request.inviteSuppliers'/>" />
		<input type="hidden" id="activityId" value="<s:property value='#request.activityId'/>" />
	<br />
	<table width="90%" class="table_search" align="center" cellpadding="0"
		...
		<tr>
			...
			<td>
				<input  id="inviteSuppliersButton" type="button" class="btngreen" style="height:30px" value="保存商家 "	onclick="saveInviteSuppliers();" />
			</td>
		</tr>
	</table>
	<!-- 数据列表区域 -->
